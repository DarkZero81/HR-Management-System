<?php $__env->startSection('title', 'الإجازات والطلبات'); ?>
...
<div class="space-y-6 mb-4">
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="<?php echo e(route('my.requests.create')); ?>" class="inline-flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-blue-500 to-teal-400 hover:from-blue-600 hover:to-teal-500 text-white rounded-xl font-semibold shadow-lg transition-all">
                <i data-lucide="plus" class="w-4 h-4"></i>
                <span class="hidden sm:inline">طلب جديد</span>
            </a>
            <?php if($isAdmin || $isManager): ?>
                <a href="<?php echo e(route($exportRoute) . '?' . http_build_query(request()->only(['status', 'transaction_type', 'from_date', 'to_date', 'search']))); ?>" class="inline-flex items-center gap-2 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-semibold shadow-lg transition-all">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div class="employee-form-card rounded-2xl p-4">
            <div class="flex items-center gap-3">
                ...
                <div class="flex-1">
                    ...
                    <?php $totalPercent = $stats['total'] > 0 ? 100 : 0; ?>
                    <div class="mt-2 h-1.5 bg-slate-200 rounded-full overflow-hidden">
                        <div class="h-full bg-blue-500 rounded-full transition-all duration-1000" style="width: <?php echo e($totalPercent); ?>%"></div>
                    </div>
                </div>
            </div>
        </div>
        <div class="employee-form-card rounded-2xl p-4">
            <div class="flex items-center gap-3">
                ...
                <div class="flex-1">
                    <p class="text-2xl font-black text-slate-800"><?php echo e($stats['pending']); ?></p>
                    <p class="text-xs text-slate-500">معلقة</p>
                    <?php $pendingPercent = $stats['total'] > 0 ? round(($stats['pending'] / $stats['total']) * 100) : 0; ?>
                    <div class="mt-2 h-1.5 bg-slate-200 rounded-full overflow-hidden">
                        <div class="h-full bg-amber-500 rounded-full transition-all duration-1000" style="width: <?php echo e($pendingPercent); ?>%"></div>
                    </div>
                </div>
            </div>
        </div>
        <div class="employee-form-card rounded-2xl p-4">
            <div class="flex items-center gap-3">
                ...
                <div class="flex-1">
                    <p class="text-2xl font-black text-slate-800"><?php echo e($stats['approved']); ?></p>
                    <p class="text-xs text-slate-500">موافق عليها</p>
                    <?php $approvedPercent = $stats['total'] > 0 ? round(($stats['approved'] / $stats['total']) * 100) : 0; ?>
                    <div class="mt-2 h-1.5 bg-slate-200 rounded-full overflow-hidden">
                        <div class="h-full bg-emerald-500 rounded-full transition-all duration-1000" style="width: <?php echo e($approvedPercent); ?>%"></div>
                    </div>
                </div>
            </div>
        </div>
        <div class="employee-form-card rounded-2xl p-4">
            <div class="flex items-center gap-3">
                ...
                <div class="flex-1">
                    <p class="text-2xl font-black text-slate-800"><?php echo e($stats['rejected']); ?></p>
                    <p class="text-xs text-slate-500">مرفوضة</p>
                    <?php $rejectedPercent = $stats['total'] > 0 ? round(($stats['rejected'] / $stats['total']) * 100) : 0; ?>
                    <div class="mt-2 h-1.5 bg-slate-200 rounded-full overflow-hidden">
                        <div class="h-full bg-rose-500 rounded-full transition-all duration-1000" style="width: <?php echo e($rejectedPercent); ?>%"></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-slate-100 p-4 md:p-6">
            <div class="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-3">
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-xs font-bold text-slate-500 bg-slate-100 px-4 py-1 rounded-full"><?php echo e($stats['total']); ?> طلب</span>
                </div>
                <div class="flex items-center gap-2">
            ...
            <div class="flex flex-wrap items-center gap-2 mb-4">
                <a href="<?php echo e(route('my.requests.index', array_merge(request()->query(), ['status' => '']))); ?>" class="inline-flex items-center gap-2 px-3 py-2 md:px-4 md:py-2.5 rounded-xl text-xs md:text-sm font-semibold transition-all <?php echo e(!($filters['status'] ?? '') ? 'bg-slate-800 text-white' : 'bg-slate-200 text-slate-700 hover:bg-slate-300'); ?>">
                    الكل
                </a>
                <a href="<?php echo e(route('my.requests.index', array_merge(request()->query(), ['status' => 'pending']))); ?>" class="inline-flex items-center gap-2 px-3 py-2 md:px-4 md:py-2.5 rounded-xl text-xs md:text-sm font-semibold transition-all <?php echo e(($filters['status'] ?? '') === 'pending' ? 'bg-amber-500 text-white' : 'bg-amber-100 text-amber-700 hover:bg-amber-200'); ?>">
                    <i data-lucide="clock" class="w-3.5 h-3.5 md:w-4 md:h-4"></i>
                    <span class="hidden sm:inline">معلقة</span>
                </a>
                <a href="<?php echo e(route('my.requests.index', array_merge(request()->query(), ['status' => 'approved']))); ?>" class="inline-flex items-center gap-2 px-3 py-2 md:px-4 md:py-2.5 rounded-xl text-xs md:text-sm font-semibold transition-all <?php echo e(($filters['status'] ?? '') === 'approved' ? 'bg-emerald-500 text-white' : 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200'); ?>">
                    <i data-lucide="check-circle" class="w-3.5 h-3.5 md:w-4 md:h-4"></i>
                    <span class="hidden sm:inline">موافق عليها</span>
                </a>
                <a href="<?php echo e(route('my.requests.index', array_merge(request()->query(), ['status' => 'rejected']))); ?>" class="inline-flex items-center gap-2 px-3 py-2 md:px-4 md:py-2.5 rounded-xl text-xs md:text-sm font-semibold transition-all <?php echo e(($filters['status'] ?? '') === 'rejected' ? 'bg-rose-500 text-white' : 'bg-rose-100 text-rose-700 hover:bg-rose-200'); ?>">
                    <i data-lucide="x-circle" class="w-3.5 h-3.5 md:w-4 md:h-4"></i>
                    <span class="hidden sm:inline">مرفوضة</span>
                </a>
            </div>

        <form method="GET" action="<?php echo e(route('my.requests.index')); ?>" class="mb-6" id="filterForm">
            <div class="employee-form-card rounded-2xl border border-slate-200 p-4 md:p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
                    <div class="relative">
                        ...
                        <input type="text" name="search" value="<?php echo e($filters['search'] ?? ''); ?>" placeholder="ابحث عن طلب..." class="employee-form-input w-full rounded-xl border border-slate-200 bg-slate-50 pr-10 pl-4 py-2.5 text-sm focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/10 transition-all">
                    </div>
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <select name="status" class="employee-form-input w-full rounded-xl border border-slate-200 bg-slate-50 pl-10 pr-8 py-2.5 text-sm focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/10 transition-all appearance-none">
                                ...
                                <option value="pending" <?php echo e(($filters['status'] ?? '') === 'pending' ? 'selected' : ''); ?>>معلقة</option>
                                <option value="approved" <?php echo e(($filters['status'] ?? '') === 'approved' ? 'selected' : ''); ?>>موافق عليها</option>
                                <option value="rejected" <?php echo e(($filters['status'] ?? '') === 'rejected' ? 'selected' : ''); ?>>مرفوضة</option>
                            </select>
                        </div>
                    </div>
                    <div>
                        ...
                        <div class="relative">
                            <input type="date" name="from_date" value="<?php echo e($filters['from_date'] ?? ''); ?>" class="employee-form-input w-full rounded-xl border border-slate-200 bg-slate-50 pr-2 pl-4 py-2.5 text-sm focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/10 transition-all">
                        </div>
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-600 mb-1.5">إلى تاريخ</label>
                        <div class="relative">
                            <input type="date" name="to_date" value="<?php echo e($filters['to_date'] ?? ''); ?>" class="employee-form-input w-full rounded-xl border border-slate-200 bg-slate-50 pr-2 pl-4 py-2.5 text-sm focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/10 transition-all">
                        </div>
                    </div>
                </div>
                    ...
                    <table class="w-full text-sm">
                            ...
                            <tr>
                                ...
                                <th class="px-4 py-4 text-slate-600 text-right font-medium cursor-pointer hover:text-cyan-600 transition-colors" onclick="sortBy('start_date_time')">
                                    <div class="flex items-center gap-1">
                                        من تاريخ
                                        <?php if(request('sort_by') === 'start_date_time'): ?>
                                            <i data-lucide="<?php echo e(request('sort_direction') === 'asc' ? 'chevron-up' : 'chevron-down'); ?>" class="w-3.5 h-3.5"></i>
                                        <?php endif; ?>
                                    </div>
                                </th>
                                <th class="px-4 py-4 text-slate-600 text-right font-medium cursor-pointer hover:text-cyan-600 transition-colors" onclick="sortBy('end_date_time')">
                                    <div class="flex items-center gap-1">
                                        إلى تاريخ
                                        <?php if(request('sort_by') === 'end_date_time'): ?>
                                            <i data-lucide="<?php echo e(request('sort_direction') === 'asc' ? 'chevron-up' : 'chevron-down'); ?>" class="w-3.5 h-3.5"></i>
                                        <?php endif; ?>
                                    </div>
                                </th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <?php $__empty_1 = true; $__currentLoopData = $transactions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $transaction): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                ...
                                ?>
                                <tr class="hover:bg-slate-50/80 transition-colors cursor-pointer" data-transaction-id="<?php echo e($transaction->id); ?>" onclick="openRequestModal(<?php echo e($transaction->id); ?>)">
                                    <td class="px-4 py-4 text-sm text-slate-500"><?php echo e($transaction->id); ?></td>
                                    <td class="px-4 py-4">
                                        <div class="flex items-center gap-3">
                                            <div class="w-8 h-8 rounded-full bg-gradient-to-br from-cyan-500 to-blue-600 flex items-center justify-center text-white font-bold text-xs shadow-sm flex-shrink-0">
                                                <?php echo e(strtoupper(substr($transaction->employee?->first_name ?? 'U', 0, 1))); ?>

                                            </div>
                                            <span class="font-semibold text-slate-800 text-sm"><?php echo e($transaction->employee?->full_name ?? '—'); ?></span>
                                        </div>
                                    </td>
                                    <td class="px-4 py-4">
                                        <div class="flex items-center gap-2">
                                            <div class="w-8 h-8 rounded-lg <?php echo e($typeConfig['bg']); ?> flex items-center justify-center flex-shrink-0">
                                                <i data-lucide="<?php echo e($typeConfig['icon']); ?>" class="w-4 h-4 <?php echo e($typeConfig['text']); ?>"></i>
                                            </div>
                                            <span class="font-semibold text-slate-800 text-sm"><?php echo e($typeConfig['label']); ?></span>
                                        </div>
                                    </td>
                                    <td class="px-4 py-4">
                                        <span class="inline-flex items-center rounded-full border px-4 py-0.5 text-xs font-bold <?php echo e($statusColors); ?>">
                                            <?php echo e(match($transaction->status) { 'pending' => 'معلقة', 'approved' => 'موافق عليها', 'rejected' => 'مرفوضة', default => $transaction->status }); ?>

                                        </span>
                                    </td>
                                    <td class="px-4 py-4 text-sm text-slate-600"><?php echo e(\Carbon\Carbon::parse($transaction->start_date_time)->format('Y-m-d')); ?></td>
                                    <td class="px-4 py-4 text-sm text-slate-600"><?php echo e(\Carbon\Carbon::parse($transaction->end_date_time)->format('Y-m-d')); ?></td>
                                    <td class="px-4 py-4 text-center text-sm font-semibold text-slate-700"><?php echo e((int) $days); ?> <span class="text-xs text-slate-400">يوم</span></td>
                                    <td class="px-4 py-4">
                                    <div class="flex items-center justify-center gap-1">
                                        <button type="button" onclick="openRequestModal(<?php echo e($transaction->id); ?>)" class="p-2 rounded-lg bg-slate-100 hover:bg-blue-100 text-slate-600 hover:text-blue-600 transition-all" title="عرض">
                                            <i data-lucide="eye" class="w-4 h-4"></i>
                                        </button>
                                        <?php if($isAdmin || $isManager): ?>
                                            <?php if($transaction->status === 'pending'): ?>
                                                <form action="<?php echo e(url($basePath . '/' . $transaction->id . '/status')); ?>" method="POST" class="inline" onclick="event.stopPropagation()" onsubmit="return confirm('هل أنت متأكد من قبول هذا الطلب؟')">
                                                    <?php echo csrf_field(); ?>
                                                ...
                                                <form action="<?php echo e(url($basePath . '/' . $transaction->id . '/status')); ?>" method="POST" class="inline" onclick="event.stopPropagation()" onsubmit="return confirm('هل أنت متأكد من رفض هذا الطلب؟')">
                                                    <?php echo csrf_field(); ?>
                                                    <?php echo method_field('PATCH'); ?>
                                            ...
                                            <?php endif; ?>
                                        <?php endif; ?>
                                        <?php if($transaction->status === 'pending' && !$isAdmin && !$isManager && $transaction->employee_id === auth()->user()?->employee?->id): ?>
                                            <form action="<?php echo e(route('my.requests.destroy', $transaction)); ?>" method="POST" class="inline" onclick="event.stopPropagation()" onsubmit="return confirm('هل أنت متأكد من إلغاء هذا الطلب؟')">
                                                <?php echo csrf_field(); ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setInterval(function() {
        fetch('<?php echo e(route('my.requests.index')); ?>', {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\DELL\Documents\Almonkez\employer_mange\resources\views/requests/index.blade.php ENDPATH**/ ?>