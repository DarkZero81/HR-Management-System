...
<div class="space-y-6 mb-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="bg-white rounded-2xl shadow-lg border border-slate-100 p-6 hover:shadow-xl transition-shadow">
            ...
            <?php else: ?>
                <form action="<?php echo e(route('attendance.checkin')); ?>" method="POST" class="space-y-4">
                    <?php echo csrf_field(); ?>
                    ...
                    <button type="submit" <?php if($hasCheckedIn): ?> disabled <?php endif; ?>
                        class="w-full inline-flex items-center justify-center mt-6 gap-2 px-6 py-3 rounded-xl font-bold shadow-lg transition-all <?php if(!$hasCheckedIn): ?> bg-gradient-to-r from-blue-500 to-teal-400 hover:from-blue-600 hover:to-teal-500 text-white shadow-blue-500/25 <?php else: ?> bg-slate-100 text-slate-500 cursor-not-allowed <?php endif; ?>">
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-slate-100 p-6 hover:shadow-xl transition-shadow">
            ...
            <?php else: ?>
                <form action="<?php echo e(route('attendance.checkout')); ?>" method="POST" class="space-y-4">
                    <?php echo csrf_field(); ?>
                    <input type="hidden" name="employee_id" value="<?php echo e($employee->id); ?>">
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-slate-100 p-6">
        <form method="GET" action="<?php echo e(route('attendance.index')); ?>" class="flex flex-col gap-4 sm:flex-row sm:items-end">
            <div class="flex-1">
            ...
            <div class="flex-1">
                ...
                <select name="status" class="w-full px-8 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-slate-800">
                    ...
                    <option value="holiday" <?php echo e(request('status') == 'holiday' ? 'selected' : ''); ?>>إجازة</option>
                </select>
            </div>
            <div class="flex items-end gap-2">
                ...
                <a href="<?php echo e(route('attendance.index')); ?>" class="px-6 py-2.5 bg-slate-200 hover:bg-slate-300 text-slate-500 font-semibold rounded-xl transition-all whitespace-nowrap">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
        ...
        <div class="bg-white rounded-2xl p-6 shadow-lg border border-slate-100 hover:shadow-xl transition-shadow">
            ...
            <p class="text-3xl font-bold text-emerald-600"><?php echo e($stats['present']); ?></p>
        </div>
        <div class="bg-white rounded-2xl p-6 shadow-lg border border-slate-100 hover:shadow-xl transition-shadow">
            <div class="flex items-center justify-between mb-3">
            ...
            <p class="text-3xl font-bold text-amber-600"><?php echo e($stats['late']); ?></p>
        </div>
        <div class="bg-white rounded-2xl p-6 shadow-lg border border-slate-100 hover:shadow-xl transition-shadow">
            <div class="flex items-center justify-between mb-3">
            ...
            <p class="text-3xl font-bold text-rose-600"><?php echo e($stats['absent']); ?></p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-slate-100 overflow-hidden">
        <div class="px-6 py-5 border-b border-slate-100 bg-gradient-to-l from-slate-50 to-white">
            <div class="flex items-center justify-between">
                ...
                <span class="rounded-full bg-blue-50 px-4 py-2 text-sm font-bold text-blue-700 border border-blue-100"><?php echo e($logs->total()); ?> سجل</span>
            </div>
        </div>
                        ...
                        <tr class="hover:bg-slate-50/80 transition-colors group">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-full bg-gradient-to-br from-cyan-500 to-blue-600 flex items-center justify-center text-white font-bold text-xs shadow-md">
                                        <?php echo e(strtoupper(substr($log->employee?->first_name ?? 'U', 0, 1))); ?>

                                    </div>
                                    <span class="font-semibold text-slate-800"><?php echo e($log->employee?->full_name ?? '—'); ?></span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center px-3 py-1 rounded-lg bg-slate-100 text-slate-500 text-sm font-medium">
                                    <?php echo e($log->log_date); ?>

                                </span>
                            </td>
                            <td class="px-6 py-4 text-slate-600"><?php echo e($log->check_in?->format('H:i') ?? '—'); ?></td>
                            <td class="px-6 py-4 text-slate-600"><?php echo e($log->check_out?->format('H:i') ?? '—'); ?></td>
                            <td class="px-6 py-4 text-slate-600"><?php echo e($log->late_minutes); ?> د</td>
                            <td class="px-6 py-4 text-slate-600"><?php echo e($log->overtime_minutes); ?> د</td>
                            <td class="px-6 py-4">
                                <?php
                                ...
                                ?>
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold <?php echo e($statusClasses); ?>">
                                    <?php echo e($statusLabel); ?>
                            ...
                            <td class="px-6 py-4">
                                ...
                                <div class="flex items-center gap-2">
                                    <a href="<?php echo e(route('attendance.edit', $log)); ?>" class="p-2 rounded-xl bg-slate-100 hover:bg-blue-100 text-slate-600 hover:text-blue-600 transition-all" title="تعديل">
                                        <i data-lucide="pencil" class="w-4 h-4"></i>
                                    </a>
                                    <form action="<?php echo e(route('attendance.destroy', $log)); ?>" method="POST" class="inline" onsubmit="return confirm('حذف هذا السجل؟');">
                                        <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\DELL\Documents\Almonkez\employer_mange\resources\views/attendance/index.blade.php ENDPATH**/ ?>