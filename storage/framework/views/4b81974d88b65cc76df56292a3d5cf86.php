<?php $__env->startSection('title', $shift->shift_name); ?>
<?php $__env->startSection('content'); ?>
<div class="space-y-6" dir="rtl">
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            <p class="text-xs font-black uppercase tracking-[0.35em] text-slate-400">الورديات</p>
            <h1 class="text-3xl font-bold text-slate-800"><?php echo e($shift->shift_name); ?></h1>
            ...
            <p class="text-sm text-slate-500 mt-1">فترة السماح للتأخير: <?php echo e($shift->grace_period_minutes); ?> دقيقة</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="<?php echo e(route('shifts.index')); ?>" class="inline-flex items-center gap-2 px-4 py-2.5 bg-slate-800 hover:bg-slate-700 text-white rounded-xl font-medium transition-all border border-white/10">
                <i data-lucide="arrow-right" class="w-4 h-4"></i>
                <span class="hidden sm:inline">العودة</span>
            </a>
            <a href="<?php echo e(route('shifts.edit', $shift)); ?>" class="inline-flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-blue-500 to-teal-400 hover:from-blue-600 hover:to-teal-500 text-white rounded-xl font-semibold shadow-lg transition-all">
    ...
    <div class="grid grid-cols-3 md:grid-cols-5 gap-4">
        <div class="employees-card rounded-2xl p-5">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-2xl font-black text-slate-800"><?php echo e($shift->employees_count); ?></p>
        ...
        <div class="employees-card rounded-2xl p-5">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-2xl font-black text-slate-800"><?php echo e($shift->grace_period_minutes); ?></p>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-slate-100">
                    <?php $__empty_1 = true; $__currentLoopData = $shift->employees; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $employee): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="employees-table-row hover:bg-slate-50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <?php if($employee->avatar): ?>
                                        <img src="<?php echo e(asset('storage/' . $employee->avatar)); ?>" class="w-10 h-10 rounded-full object-cover border border-white/10 shadow-sm">
                                    <?php else: ?>
                                        <div class="w-10 h-10 rounded-full bg-gradient-to-br from-cyan-500 to-blue-600 flex items-center justify-center text-white font-bold text-sm shadow-sm">
                                            <?php echo e(strtoupper(substr($employee->first_name ?? 'U', 0, 1))); ?>

                                        </div>
                                    <?php endif; ?>
                                    <div>
                                        <div class="font-semibold text-slate-800"><?php echo e($employee->full_name); ?></div>
                                        <div class="text-xs text-slate-500"><?php echo e($employee->user?->email ?? 'لا يوجد حساب مرتبط'); ?></div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-slate-600 text-sm"><?php echo e($employee->department?->name ?? '-'); ?></td>
                            <td class="px-6 py-4 text-slate-600 text-sm"><?php echo e($employee->job_title ?? '-'); ?></td>
                            <td class="px-6 py-4 text-slate-600 text-sm"><?php echo e($employee->join_date?->format('Y-m-d') ?? '-'); ?></td>
                            <td class="px-6 py-4">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\DELL\Documents\Almonkez\employer_mange\resources\views/shifts/show.blade.php ENDPATH**/ ?>