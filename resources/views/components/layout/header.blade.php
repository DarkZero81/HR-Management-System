        ...
        <div class="relative">
            <button id="notificationsToggleDesktop" class="p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors relative" title="الإشعارات">
                ...
                @if($unreadNotifications > 0)
                    <span class="absolute -top-0.5 -left-0.5 w-4 h-4 bg-red-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center">{{ $unreadNotifications }}</span>
                @endif
            </button>
            <div id="notificationsDropdownDesktop" class="hidden absolute top-full left-0 mt-2 w-80 bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-2xl shadow-2xl overflow-hidden z-50">
                <div class="p-3 border-b border-slate-100 dark:border-white/10 flex items-center justify-between">
                    ...
                    <a href="{{ route('my.requests.index') }}" class="text-xs text-cyan-600 dark:text-cyan-400 hover:text-cyan-700 font-medium">عرض الكل</a>
                </div>
                <div class="max-h-72 overflow-y-auto">
                    @forelse($latestRequests as $req)
                        <a href="{{ route('my.requests.index') }}" class="block px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors border-b border-slate-100 dark:border-white/5 last:border-0">
                            <div class="flex items-center justify-between">
                                <span class="text-sm font-semibold text-slate-800 dark:text-white">{{ match($req->transaction_type) { 'leave' => 'إجازة', 'permission' => 'إذن', 'promotion' => 'ترقية', 'penalty' => 'جزاء', 'transfer' => 'نقل', default => $req->transaction_type } }}</span>
                                <span class="text-[10px] px-2 py-0.5 rounded-full {{ $req->status === 'pending' ? 'bg-amber-100 text-amber-700' : ($req->status === 'approved' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700') }}">{{ $req->status }}</span>
                            </div>
                            <p class="text-xs text-slate-400 dark:text-slate-500 mt-1">{{ $req->start_date_time?->format('Y-m-d') ?? '—' }}</p>
                        </a>
                    @empty
        ...
        <div class="relative">
            <button id="profileToggleDesktop" class="flex items-center gap-3 p-1.5 pr-3 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                <div class="w-8 h-8 rounded-full bg-gradient-to-br from-cyan-500 to-blue-600 flex items-center justify-center">
                    <span class="text-xs font-black text-white">{{ strtoupper(substr($user->email ?? 'U', 0, 1)) }}</span>
                </div>
                <div class="text-right hidden xl:block">
                    <p class="text-sm font-bold text-slate-800 dark:text-white leading-tight">{{ $user->name ?? $user->email }}</p>
                    <p class="text-[10px] font-semibold text-slate-400 dark:text-slate-500 leading-tight">{{ $userRole }}</p>
                </div>
            ...
            <div id="profileDropdownDesktop" class="hidden absolute top-full left-0 mt-2 w-56 bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-2xl shadow-2xl overflow-hidden z-50">
                ...
                <div class="p-2">
                    <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors text-slate-700 dark:text-slate-200">
