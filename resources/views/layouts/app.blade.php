    ...
    <div class="flex min-h-screen flex-col gap-4 p-3 lg:flex-row lg:p-4">
        <aside class="w-50 shrink-0 rounded-[32px] border border-white/10 bg-slate-950/80 p-4 shadow-[0_25px_80px_-35px_rgba(8,15,30,0.8)] backdrop-blur-2xl lg:w-72 lg:p-5">
            <div class="flex items-center justify-between border-b border-white/10 pb-4 lg:block">
                ...
                <div class="rounded-full border border-cyan-400/30 bg-cyan-500/10 px-3 py-1 text-[10px] font-bold text-cyan-300">Dark UI</div>
            </div>
            ...
            <nav class="mt-6 space-y-1.5">
                ...
                @foreach($nav as $item)
                    <a href="{{ route($item['route']) }}" class="flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-bold transition duration-200 {{ request()->routeIs($item['route']) ? 'bg-gradient-to-l from-cyan-500 to-blue-600 text-white shadow-lg shadow-cyan-500/20' : 'text-slate-400 hover:bg-white/5 hover:text-white' }}">
            ...
            <div class="mt-auto space-y-4 rounded-[28px] border border-white/10 bg-white/5 p-4">
                <div class="rounded-2xl border border-cyan-400/20 bg-gradient-to-br from-slate-900 to-slate-800 p-3">
                    ...
                    <p class="mt-2 text-[11px] leading-relaxed text-slate-400">تابع الحضور والطلبات والرواتب بواجهة حديثة ومناسبة للعمل العربي.</p>
                </div>

                <div class="flex items-center gap-3 rounded-2xl border border-white/10 bg-slate-900/60 p-3">
                    <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-cyan-500 to-blue-600 font-black text-white">
                    ...
                    <div class="overflow-hidden">
                        ...
                        <p class="mt-1 text-[11px] font-semibold text-slate-400">{{ optional(auth()->user()->role)->role_name ?? 'موظف' }}</p>
                    </div>
                </div>
            </div>
        </aside>

        <main class="flex-1 overflow-y-auto rounded-[32px] border border-white/10 bg-slate-950/40 p-4 shadow-[0_20px_70px_-35px_rgba(15,23,42,0.85)] backdrop-blur-2xl lg:p-6">
