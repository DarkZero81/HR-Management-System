...
<div class="space-y-6">
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('employees.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-blue-500 to-teal-400 hover:from-blue-600 hover:to-teal-500 text-white rounded-xl font-semibold shadow-lg transition-all">
                <i data-lucide="plus" class="w-4 h-4"></i>
                <span class="hidden sm:inline">إضافة موظف</span>
            </a>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-slate-100 p-6">
        <form action="{{ route('employees.index') }}" method="GET" class="grid grid-cols-1 gap-4 md:grid-cols-3">
            <div>
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="الاسم، الكنية، أو الرقم الوطني..." class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-slate-800">
            </div>
            <div>
                <label class="block text-sm font-medium text-slate-700 mb-2">تصفية حسب القسم</label>
                <select name="department_id" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all cursor-pointer text-slate-800">
            ...
            <div class="flex items-end gap-2">
                <button type="submit" class="flex-1 px-4 py-3 bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-medium transition-all">
                    تطبيق الفلترة
                </button>
                @if(request()->has('search') || request()->has('department_id'))
                    <a href="{{ route('employees.index') }}" class="px-4 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-medium transition-all">
                        إعادة
                    </a>
                @endif
            </div>
                        ...
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('employees.edit', $employee->id) }}" class="p-2 rounded-xl bg-slate-100 hover:bg-blue-100 text-slate-600 hover:text-blue-600 transition-colors" title="تعديل">
