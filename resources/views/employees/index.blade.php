...
@section('content')
<div class="space-y-6">
    <div class="flex flex-col gap-4 rounded-[28px] border border-slate-200/70 bg-white/90 p-6 shadow-[0_20px_60px_-35px_rgba(15,23,42,0.45)] backdrop-blur lg:flex-row lg:items-center lg:justify-between">
        <div>
        ...
        <a href="{{ route('employees.create') }}" class="inline-flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-l from-cyan-500 to-blue-600 px-5 py-3 text-sm font-semibold text-white transition hover:opacity-90 shadow-lg">
            <i data-lucide="user-plus" class="h-4 w-4"></i>
            إضافة موظف جديد
        </a>
    </div>

    <div class="overflow-hidden rounded-[28px] border border-slate-200/70 bg-white/90 shadow-[0_20px_60px_-35px_rgba(15,23,42,0.45)] backdrop-blur">
                ...
                <tbody class="divide-y divide-slate-100 bg-white">
                    @forelse($employees as $employee)
                        <tr class="transition hover:bg-slate-50">
                            ...
                            <td class="px-5 py-4 text-slate-600">{{ $employee->user->email ?? '—' }}</td>
                            <td class="px-5 py-4 text-slate-600">{{ $employee->phone ?? '—' }}</td>
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('employees.show', $employee) }}" class="inline-flex items-center gap-1 rounded-full bg-slate-100 px-3 py-1.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-200">
                                        <i data-lucide="eye" class="h-3 w-3"></i>عرض
                                    </a>
                                    <a href="{{ route('employees.edit', $employee) }}" class="inline-flex items-center gap-1 rounded-full bg-blue-50 px-3 py-1.5 text-sm font-semibold text-blue-700 transition hover:bg-blue-100">
                                    ...
                                    <form action="{{ route('employees.destroy', $employee) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من حذف هذا الموظف؟')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="inline-flex items-center gap-1 rounded-full bg-rose-50 px-3 py-1.5 text-sm font-semibold text-rose-700 transition hover:bg-rose-100">
                    ...
                    @empty
                        <tr><td colspan="7" class="px-5 py-8 text-center text-slate-500">لا توجد موظفين مسجلين حتى الآن.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
