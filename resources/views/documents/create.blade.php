...
<div class="max-w-3xl mx-auto space-y-6 px-4 py-4" dir="rtl">
    ...
    <form method="POST" action="{{ route('documents.store') }}" enctype="multipart/form-data" class="employee-form-card rounded-[28px] border border-white/10 dark:border-white/5 p-6 space-y-5 shadow-2xl backdrop-blur-md">
        @csrf
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
            <div>
                ...
                <select name="employee_id" id="employee_id" required class="employee-form-input w-full rounded-xl border border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-slate-950/60 text-sm text-slate-800 dark:text-slate-200 px-3 py-2.5 focus:outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/10 transition-all cursor-pointer">
                    <option value="" class="bg-white dark:bg-slate-900 text-slate-500 dark:text-slate-400">اختر الموظف...</option>
                    @foreach($employees as $employee)
                        <option value="{{ $employee->id }}" {{ old('employee_id') == $employee->id ? 'selected' : '' }} class="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200">{{ $employee->full_name }}</option>
                    @endforeach
                </select>
                @error('employee_id')
            ...
            <div>
                ...
                <select name="document_type" id="document_type" required class="employee-form-input w-full rounded-xl border border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-slate-950/60 text-sm text-slate-800 dark:text-slate-200 px-3 py-2.5 focus:outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/10 transition-all cursor-pointer">
                    ...
                    <option value="identity" {{ old('document_type') == 'identity' ? 'selected' : '' }} class="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200">هوية</option>
                    <option value="passport" {{ old('document_type') == 'passport' ? 'selected' : '' }} class="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200">جواز سفر</option>
                    <option value="contract" {{ old('document_type') == 'contract' ? 'selected' : '' }} class="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200">عقد</option>
                    <option value="health_certificate" {{ old('document_type') == 'health_certificate' ? 'selected' : '' }} class="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200">شهادة صحية</option>
                </select>
                @error('document_type')
                    <p class="text-rose-500 text-sm mt-1.5 flex items-center gap-1.5">
                        <i data-lucide="alert-circle" class="w-4 h-4"></i>
