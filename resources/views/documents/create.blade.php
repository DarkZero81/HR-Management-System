...
@section('title', $myMode ? 'رفع وثيقة جديدة' : 'إضافة وثيقة جديدة')
...
<div class="max-w-3xl mx-auto space-y-6 px-4 py-4" dir="rtl">
    <div class="border-b border-white/5 pb-4">
        ...
        <h1 class="text-3xl font-bold text-slate-800">{{ $myMode ? 'رفع وثيقة جديدة' : 'إضافة وثيقة جديدة' }}</h1>
        <p class="text-sm text-slate-400 dark:text-slate-500 mt-1">{{ $myMode ? 'ارفع وثيقة خاصة بك (هوية، جواز، عقد، شهادة صحية).' : 'أدخل بيانات الوثيقة وارفق الملف.' }}</p>
    </div>
    ...
    @php
        $oldVal = fn(string $key) => old($key, '');
        $oldDateVal = fn(string $key) => old($key, '');
        $showEmployeeField = !$myMode;
        $fileInputName = $myMode ? 'document' : 'file';
        $cancelRoute = $myMode ? 'my.documents.index' : 'documents.index';
    @endphp

    <form method="POST" action="{{ $myMode ? route('my.documents.store') : route('documents.store') }}" enctype="multipart/form-data" class="employee-form-card rounded-[28px] border border-white/10 dark:border-white/5 p-6 space-y-5 shadow-2xl backdrop-blur-md">
        @csrf
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
            ...
            <div class="sm:col-span-2">
                ...
                <div class="relative border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-xl p-6 text-center hover:border-cyan-500 transition-colors cursor-pointer">
                    <input type="file" name="{{ $fileInputName }}" id="documentFile" accept=".pdf,.jpg,.jpeg,.png" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" onchange="updateFileName(this)">
        ...
        <div class="flex items-center justify-end gap-2.5 border-t border-slate-200 dark:border-white/5 pt-4">
            ...
            <button type="submit" class="rounded-xl bg-gradient-to-l from-cyan-500 to-blue-600 hover:opacity-95 px-4 py-2.5 text-xs font-bold text-white shadow-lg shadow-blue-600/10 transition active:scale-95 cursor-pointer">
                حفظ الوثيقة
            </button>
        </div>
    </form>
